...
3. Реферер указателя при покупке PLAY в контракте Exchange. После того как реферер однажды указан, он неизменяем в контракте. Реферер единый для всей сети контрактов (всех версий игр). Дальнейшие проекты (по типу NFT допустим) можно также привязывать к данной реферальной системе. Все данные о рефке хранятся на отдельном смарт-контракте (RefStorage). Реферальная глубина неограниченна.
